...
namespace NWH.WheelController3D
{
    ...
    public partial class WheelController : WheelUAPI
    {
        ...
        public void Initialize()
        {
            ...
            if (wheel.radius < Vehicle.SMALL_NUMBER) wheel.radius = Vehicle.SMALL_NUMBER;
        ...
        public override void Step()
        {
            ...
            // Update wheel transforms with the new suspension tranform
            UpdateWheelTransforms();
            ...
            // Set up ignore layers since sphere cast does not have masking options
            ...
            if (_isGrounded)
            {
                ...
                spring.length = Mathf.Clamp(-localGroundedYPosition, 0f, spring.maxLength);
            }
            else
            {
                spring.length = spring.maxLength;
            ...
            /////////////////// FORCES
            // Update axle torque
            Vector3 torque = Vector3.Cross(HitPoint - TargetRigidbody.centerOfMass, forwardFriction.force * _wheelRight);
            ...
            wheel.nonRotatingContainer.SetPositionAndRotation(_wheelWorldPosition, _wheelWorldRotation);
            wheel.meshCollider.transform.SetPositionAndRotation(_suspensionWorldPosition, _suspensionWorldRotation);
        }


        private void UpdateSuspensionTransforms()
        {
            WheelControllerUtility.CalculateRotationMatrix(steerAngle, ref steerRotationMatrix, ref steerRotationQuaternion);

            // Adjust the matrix to rotate around the suspension's local origin
        ...
        /// <summary>
        /// Intializes the layers by storing all the colliders on the vehicle and their layers
        /// so that they can be set to ignore raycast before casting and then restored later.
        /// Call if the vehicle colliders change.
        /// </summary>
        ...
        private void SetColliderLayersToIgnore()
        {
            ...
            {
                if (_vehicleColliderObjects[i] != null)
                {
